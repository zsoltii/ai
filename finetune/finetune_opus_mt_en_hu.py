...
from transformers import (
    ...
    AutoTokenizer,
    BitsAndBytesConfig,
    AutoConfig, M2M100ForConditionalGeneration, M2M100Tokenizer, AutoModelForSeq2SeqLM
)
...
def generate_translation_dataset(sentence_count):
    ...
    for i in range(sentence_count):
        ...
        opus_inputs = opus_tokenizer(en_sentence, return_tensors="pt").to(opus_model.device)
        print("Angol mondat fordítása opus modell segítségével folyamatban...")
        with torch.no_grad():
            opus_outputs = opus_model.generate(**opus_inputs,
                                                   max_length=1024)
        hu_sentence = opus_tokenizer.batch_decode(opus_outputs, skip_special_tokens=True)[
    ...
    del opus_model, opus_tokenizer, sentence_model_config, sentence_model_quantization_config, sentence_model, sentence_tokenizer
